...
NumericMatrix HollandWindProfilePi(NumericVector f, NumericVector vMax, NumericVector rMax, NumericVector dP, float rho, NumericVector beta, NumericVector R)
{
  ...
  float Vi, Ri,fi,vMaxi,rMaxi,betai,dPi, Zi;
    ...
    dVm = (-abs(fi)/2.0f + (E * (fi*fi) * rMaxi * sqrtf((4.0f * betai * dPi / rho) / E + (fi * rMaxi) * (fi * rMaxi))) / (2.0f * (4.0f * betai * dPi / rho + E * (fi * rMaxi) * (fi * rMaxi))));
		...
		else
		{
		  //dPi = dPi*100.0f; // !need to multiply by dPi 100 to get to Pa not in TCRM  https://github.com/GeoscienceAustralia/tcrm/blob/1916233f7dfdecf6a1b5f5b0d89f3eb1d164bd3e/wind/windmodels.py#L381
			Vi =  (sqrtf((dPi * betai / rho) * delta * edelta + (Ri * fi / 2.0f)*(Ri * fi / 2.0f)) - Ri *abs(fi) / 2.0f);
			//Zi = ((sqrtf((dPi * betai / rho) * delta * edelta + (Ri * fi / 2.0f)*(Ri * fi / 2.0f))) / Ri - abs(fi) + edelta * (2.0f * (betai * betai) * dPi * (delta - 1.0f) * delta + rho * edelta * (fi * Ri) *(fi * Ri)) / (2.0f * rho * Ri * sqrtf(4.0f * (betai * dPi / rho) * delta * edelta + (fi * Ri) *(fi * Ri))));
		  Zi = abs(fi) + (betai*betai * dPi * (delta * delta) * edelta / (2.0f * rho * Ri) - betai*betai * dPi * delta * edelta / (2.0f * rho * Ri) + Ri * fi * fi / 4.0f) / sqrtf(betai * dPi * delta * edelta / rho + (Ri * fi / 2)*(Ri * fi / 2)) + (sqrtf(betai * dPi * delta * edelta / rho + (Ri * fi / 2)*(Ri * fi / 2))) / Ri;
		}
		VZ(i,0) = Vi*fi / abs(fi);
		VZ(i,1) = Zi*fi / abs(fi);
	}
  ...
  NumericMatrix VZ(n,2);
	...
	for(int i = 0;i < n; i++){
	  ...
	  //TClati = TClat[i];
		...
		Bs = beta[i];//(-0.000044f * powf(dPi / 100.0f, 2.0f) + 0.01 * (dPi / 100.0f) - 0.014f * abs(TClati) + 1.0);
		...
		VZ(i,0) = (fi / abs(fi)) * vMaxi * pow( delta * edelta, xx);
...
NumericMatrix DoubleHollandWindProfilePi(NumericVector f, NumericVector vMax, NumericVector rMax, NumericVector dP, NumericVector cP, float rho, NumericVector beta, NumericVector R)
{
  ...
  //missing powf on second line
	d2Vm = (-1.0f / (8.0f * pow(4.0f * beta1 * dp1 / (rho * E) + (4.0f * beta2 * dp2 / rho) * nu * exp(-nu) + powf(rMax1 * fi, 2.0f), 1.5f))*
		...
		Vi = (fi / abs(fi) * sqrt(gradientV1 + gradientV2 + (Ri *fi / 2.0f) *(Ri *fi / 2.0f)) - Ri * abs(fi) / 2.0f);

		if (dPi >= 1500.0f && Ri <= rMax1)
		{
			Vi = (fi / abs(fi) * Ri * (Ri * (Ri * aa + bb) + cc));
		}
...
NumericMatrix HubbertWindFieldPi(NumericVector f, NumericVector rMax, NumericVector vFm, NumericVector thetaFm, NumericMatrix Rlam, NumericVector V)
{
  ...
  float sf;
	...
	for(int i = 0;i < n; i++){
	  ...
	  vFmi = vFm[i];
	  rMaxi = rMax[i];
	  thetaFmRAD = thetaFm[i]*pi / 180.0f;
		//V = self.velocity(R)
		Ri = Rlam(i,0);
    ...
    //CB didn't have * pi / 180.0f
		thetaMaxAbsolute = thetaFmRAD + thetaMax*-1.0f*sf* pi / 180.0f;
  ...
  sf = f/abs(f);
	for(int i = 0;i < n; i++){
	  ...
	  vFmi = vFm[i];
		...
		inflow = inflow*pi / 180.0f;  //missing -sign(f) i.e. f/abs(f)
		...
		if (Vsf >= 6.0f)
		{
			swrf = 0.81f - (2.93f * abs(Vsf - 6.0f) / 1000.0f);
		}
		if (Vsf >= 19.5f)
		{
			swrf = 0.77f - (4.31f * abs(Vsf - 19.5f) / 1000.0f);
		}
		if (Vsf > 45.0f)
		{
			swrf = 0.66f;
...
NumericMatrix KepertWindFieldPi(NumericVector rMax, NumericVector vMax, NumericVector vFm, NumericVector thetaFm, NumericVector f, NumericMatrix Rlam, NumericMatrix VZ)
{
  ...
  NumericMatrix UwVw(n,2);
	...
	float A0r, A0i,u0s,v0s,Amr,Ami,ums,vms,Apr,Api,ups,vps,Vmi,vFmi,Umod;
  ...
  //Vm = abs(max(V));
    ...
    Umod = vFmi;
    vMaxi = vMax(i);
    Vmi = abs(vMaxi);  //maximum V at rMax
    if((vFmi > 0) and ((Vmi/vFmi) < 5.0f)){
      Umod = vFmi*abs(1.25f * (1.0f - (vFmi / Vmi)));
    }
		...
		chi = abs((Cd / K) * Vi / sqrtf(sqrt(al * be)));
		eta = abs((Cd / K) * Vi / sqrtf(sqrt(al * be) + abs(gam)));
		psi = abs((Cd / K) * Vi / sqrtf(abs(sqrt(al * be) - abs(gam))));
		...
		usf = us + Vt * cosf(lami - thetaFmi);
		vsf = vs - Vt * sinf(lami - thetaFmi);
		phi = atan2f(usf, vsf);
	  //if(phi > pi){
	  //  phi = pi - phi;
	  //}
		UwVw(i,0) = sqrtf(usf*usf + vsf *vsf) * sinf(phi - lami);
		UwVw(i,1) = sqrtf(usf*usf + vsf *vsf) * cosf(phi - lami);
...
NumericMatrix Rdist(NumericVector Gridlon, NumericVector Gridlat, float TClon, float TClat)
{
  ...
  for(int i = 0;i < n; i++){
    ...
    float lat2 = Gridlat[i] * pi / 180.0f;
    float lon1 = TClon * pi / 180.0f;
    float lon2 = Gridlon[i] * pi / 180.0f;
...
NumericMatrix HollandWindProfile(float f, float vMax, float rMax, float dP, float rho, float beta, NumericVector R)
{
  ...
  for(int i = 0;i < n; i++){
    ...
    dVm = (-abs(f)/2.0f + (E * (f*f) * rMax * sqrtf((4.0f * beta * dP / rho) / E + (f * rMax) * (f * rMax))) / (2.0f * (4.0f * beta * dP / rho + E * (f * rMax) * (f * rMax))));
    ...
    else
    {
      ...
      Zi = abs(f) + (beta*beta * dP * (delta * delta) * edelta / (2.0f * rho * Ri) - beta*beta * dP * delta * edelta / (2.0f * rho * Ri) + Ri * f * f / 4.0f) / sqrtf(beta * dP * delta * edelta / rho + (Ri * f / 2)*(Ri * f / 2)) + (sqrtf(beta * dP * delta * edelta / rho + (Ri * f / 2)*(Ri * f / 2))) / Ri;
    }
    VZ(i,0) = Vi*f / abs(f);
    VZ(i,1) = Zi*f / abs(f);
  }
...
NumericMatrix NewHollandWindProfile(float f, float rMax, float dP, float rho, NumericVector R, float vMax, float beta)
{
  ...
  for(int i = 0;i < n; i++){
    //
    Ri = R[i];
    //(-0.000044f * powf(dPi / 100.0f, 2.0f) + 0.01 * (dPi / 100.0f) - 0.014f * abs(TClati) + 1.0);
...
NumericMatrix HubbertWindField(float f, float rMax, float vFm, float thetaFm, NumericMatrix Rlam, NumericVector V)
{
  ...
  float Km = 0.70; //reduction factor
  ...
  for(int i = 0;i < n; i++){
    ...
    inflow = inflow * pi / 180.0f;
    //CB didn't have * pi / 180.0f
    thetaMaxAbsolute = thetaFmRAD + thetaMax*-1.0*sf* pi / 180.0f;
...
NumericMatrix KepertWindField(float rMax, float vMax, float vFm, float thetaFm, float f, NumericMatrix Rlam, NumericMatrix VZ)
{
  ...
  float A0r, A0i,u0s,v0s,Amr,Ami,ums,vms,Apr,Api,ups,vps,Vmi,Umod;
  float us, vs, usf, vsf,phi;
  float pi = 3.141592f;

  fs = f/abs(f);
  Umod = vFm;
  Vmi = abs(vMax);  //maximum V at rMax
  //Vm = abs(max(V));
  thetaFm = thetaFm*pi / 180.0f;
  if((vFm > 0) and ((Vmi/vFm) < 5.0f)){
    Umod = vFm*abs(1.25f * (1.0f - (vFm / Vmi)));
  }
  Vt = Umod;
  ...
  for(int i = 0;i < n; i++){
    ...
    ums = (Amr * (cosf(-(lami- thetaFm) * fs) + 0.0f * sinf(-(lami- thetaFm) * fs))) * albe;  //https://en.wikipedia.org/wiki/Euler%27s_formula
    vms = (Ami * (cosf(-(lami- thetaFm) * fs) + 1.0f * sinf(-(lami- thetaFm) * fs))) * fs;
    ...
    //ups = (Apr * expf(0.0f * lami * fs)) * albe;
    //vps = (Api * expf(1.0f * lami * fs)) * fs;
    ups = (Apr * (cosf((lami- thetaFm) * fs) + 0.0f * sinf((lami- thetaFm) * fs))) * albe;  //https://en.wikipedia.org/wiki/Euler%27s_formula
    vps = (Api * (cosf((lami- thetaFm) * fs) + 1.0f * sinf((lami- thetaFm) * fs))) * albe;
